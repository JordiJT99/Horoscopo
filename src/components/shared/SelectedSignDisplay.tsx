  ...
  return (
    <div className="flex flex-col items-center text-center py-4">
      ...
      <div className="relative w-32 h-32 sm:w-36 sm:h-36 mb-4">
        {/* Replace with actual sign image or more styled icon */}
        <Image 
            src={`https://placehold.co/150x150/7c3aed/ffffff.png?text=${selectedSign.name.substring(0,1)}`} // Simple placeholder
            alt={dictionary[selectedSign.name] || selectedSign.name} 
            width={144} 
            height={144}
            className="rounded-full object-cover border-4 border-primary shadow-lg"
            data-ai-hint={`${selectedSign.name} zodiac symbol vibrant illustration`}
        />
      </div>
